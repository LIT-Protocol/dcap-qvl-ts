...
describe('crypto-utils', () => {
  ...
  describe('verifyEcdsaSignature', () => {
    it('verifies a valid ECDSA P-256 signature and fails on tampered signature (from generated vector)', () => {
      ...
      const pubPem = fs.readFileSync(path.join(process.cwd(), 'tests/ecdsa-pub.pem'), 'utf8');
      const message = fs.readFileSync(path.join(process.cwd(), 'tests/ecdsa-message.txt'));
      const sigRawHex = fs
        .readFileSync(path.join(process.cwd(), 'tests/ecdsa-sig-raw.hex'), 'utf8')
        .trim();
